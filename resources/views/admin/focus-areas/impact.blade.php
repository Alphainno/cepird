...
<div class="space-y-6">
    ...
    <!-- Create Metric (hidden until button click) -->
    <div class="bg-white rounded-sm shadow-sm border border-slate-200">
        <div class="p-6 border-b border-slate-200 flex items-center justify-between">
            ...
            <button type="button" id="showMetricFormBtn" class="px-4 py-2 bg-blue-600 text-white font-semibold rounded-sm hover:bg-blue-700 transition-colors">Add Metric</button>
        </div>
        <form id="metricCreateForm" action="{{ route('admin.impact-metrics.store') }}" method="POST" class="p-6 space-y-4 hidden">
            @csrf
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Title <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., Entrepreneurs Supported" required>
                </div>
                <div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-sm hover:bg-blue-700 transition-colors">Submit Metric</button>
            </div>
        </form>
    </div>
...
<script>
    (function() {
        const btn = document.getElementById('showMetricFormBtn');
        const form = document.getElementById('metricCreateForm');
        if (btn && form) {
