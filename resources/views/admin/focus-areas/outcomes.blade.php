...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-sm shadow-sm border border-slate-200">
        ...
        <form action="{{ route('admin.focus-area-outcomes.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Title <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title', $prefill->title ?? 'Policy Frameworks') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Icon</label>
                    <input type="text" name="icon" value="{{ old('icon', $prefill->icon ?? '📋') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., 📋">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Description</label>
                <textarea name="description" rows="2" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Short description">{{ old('description', $prefill->description ?? 'Actionable policy recommendations for government and institutions') }}</textarea>
            </div>
            <div class="grid md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Bullet 1</label>
                    <input type="text" name="bullet1" value="{{ old('bullet1', $prefill->bullet1 ?? 'Digital Transformation Strategy') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus-border-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Bullet 2</label>
                    <input type="text" name="bullet2" value="{{ old('bullet2', $prefill->bullet2 ?? 'Startup Ecosystem Policy') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus-border-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Bullet 3</label>
                    <input type="text" name="bullet3" value="{{ old('bullet3', $prefill->bullet3 ?? 'SME Development Guidelines') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus-border-blue-500">
                </div>
            </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-sm hover:bg-blue-700 transition-colors">Add Outcome</button>
            </div>
        </form>
    </div>
