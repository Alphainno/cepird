...
@section('title', 'Edit Program Item - Admin')

@section('content')
<div class="mb-8">
    <div class="flex items-center gap-4">
        <a href="{{ route('admin.program-sections.show', $item->section) }}" class="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Edit Program Item</h1>
            <p class="text-gray-600 mt-1">Update {{ $item->title }}</p>
        </div>
    </div>
</div>
    ...
    <form id="itemForm" action="{{ route('admin.program-items.update', $item) }}" method="POST" class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="info_label" id="info_label" value="{{ $item->info_label }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            <div>
                ...
                <input type="text" name="info_value" id="info_value" value="{{ $item->info_value }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                    placeholder="Describe this program item...">{{ $item->description }}</textarea>
            </div>

            <!-- Features Title -->
            <div>
                <label for="features_title" class="block text-sm font-semibold text-gray-700 mb-2">Features Section Title</label>
                <input type="text" name="features_title" id="features_title" value="{{ $item->features_title ?? 'Key Features:' }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            ...
            <div>
                ...
                <input type="number" name="order" id="order" min="0" value="{{ $item->order }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                    placeholder="0">
            </div>
                    ...
                    @if($item->features && count($item->features) > 0)
                        @foreach($item->features as $feature)
                            <div class="flex gap-2">
                                <input type="text" name="features[]" value="{{ $feature }}" class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Enter a feature point">
                ...
                <div id="metadata-container" class="space-y-2">
                    @if($item->metadata && count($item->metadata) > 0)
                        @foreach($item->metadata as $key => $value)
                            <div class="flex gap-2">
                                <input type="text" name="metadata_keys[]" value="{{ $key }}" class="w-1/3 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Label (e.g., Target)">
                                <input type="text" name="metadata_values[]" value="{{ $value }}" class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" placeholder="Value (e.g., Early-stage startups)">
        ...
        <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-4">
            <a href="{{ route('admin.program-sections.show', $item->section) }}" class="px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-lg hover:bg-gray-200 transition-colors">
                Cancel
            </a>
...
document.getElementById('itemForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = '{{ route("admin.program-sections.show", $item->section) }}';
            }, 1000);
        } else {
