...
@section('title', 'Edit Impact Stat - Admin')
...
<div class="mb-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Edit Impact Stat</h1>
            <p class="text-gray-600 mt-1">Update {{ $stat->label }}</p>
        </div>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <form action="{{ route('admin.program-impact.update', $stat) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="value" id="value" required value="{{ $stat->value }}"
            ...
            <div>
                ...
                <input type="text" name="label" id="label" required value="{{ $stat->label }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="description" id="description" value="{{ $stat->description }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div>
                ...
                <select name="color_theme" id="color_theme" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="blue" {{ $stat->color_theme === 'blue' ? 'selected' : '' }}>Blue</option>
                    <option value="teal" {{ $stat->color_theme === 'teal' ? 'selected' : '' }}>Teal</option>
                    <option value="amber" {{ $stat->color_theme === 'amber' ? 'selected' : '' }}>Amber</option>
                    <option value="indigo" {{ $stat->color_theme === 'indigo' ? 'selected' : '' }}>Indigo</option>
                </select>
            </div>

            <div>
                <label for="order" class="block text-sm font-semibold text-gray-700 mb-2">Display Order</label>
                <input type="number" name="order" id="order" min="0" value="{{ $stat->order }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder="0">
