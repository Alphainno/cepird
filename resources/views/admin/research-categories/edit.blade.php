...
@section('title', 'Edit Research Category')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Edit Research Category</h1>
            <p class="text-slate-600 mt-1">Update category details</p>
        </div>
    ...
    <div class="bg-white rounded-sm shadow-sm border border-slate-200">
        <form action="{{ route('admin.research-categories.update', $category) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter category name" required>
                <p class="mt-1 text-xs text-slate-500">The slug will be updated automatically if you change the name</p>
                @error('name')
            ...
            <!-- Slug (Read-only) -->
            <div>
                <label for="slug" class="block text-sm font-medium text-slate-700 mb-2">Slug (Auto-generated)</label>
                <input type="text" id="slug" value="{{ $category->slug }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm bg-slate-50 cursor-not-allowed" disabled>
            </div>
            ...
            <div>
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $category->order) }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="0" min="0">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
