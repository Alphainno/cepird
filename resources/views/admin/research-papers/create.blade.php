...
@section('title', 'Add Research Paper')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Add Research Paper</h1>
            <p class="text-slate-600 mt-1">Create a new research paper entry</p>
        </div>
    ...
    <div class="bg-white rounded-sm shadow-sm border border-slate-200">
        <form action="{{ route('admin.research-papers.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter paper title" required>
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="authors" id="authors" value="{{ old('authors') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., John Doe, Jane Smith" required>
                @error('authors')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter paper description" required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="publication_date" id="publication_date" value="{{ old('publication_date') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div>
                    ...
                    <input type="number" name="pages" id="pages" value="{{ old('pages') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., 12" min="1">
                    @error('pages')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" name="citations" id="citations" value="{{ old('citations', 0) }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="0" min="0">
                    @error('citations')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <input type="text" name="tags" id="tags" value="{{ old('tags') }}" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter tags separated by commas (e.g., Climate Change, Policy, Research)">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="icon_color" id="icon_color" class="w-full px-4 py-2 border border-slate-300 rounded-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                        <option value="blue-900" {{ old('icon_color') == 'blue-900' ? 'selected' : '' }}>Blue</option>
                        <option value="purple-900" {{ old('icon_color') == 'purple-900' ? 'selected' : '' }}>Purple</option>
                        <option value="teal-900" {{ old('icon_color') == 'teal-900' ? 'selected' : '' }}>Teal</option>
                        <option value="red-900" {{ old('icon_color') == 'red-900' ? 'selected' : '' }}>Red</option>
                        <option value="green-900" {{ old('icon_color') == 'green-900' ? 'selected' : '' }}>Green</option>
                        <option value="yellow-900" {{ old('icon_color') == 'yellow-900' ? 'selected' : '' }}>Yellow</option>
                    </select>
                    @error('icon_color')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Upload PDF (max 10MB)</p>
                @error('pdf_file')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-3">
                <div class="flex items-center">
                    <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
