...
document.querySelectorAll('.md\\:flex button[onclick^="scrollToSection"]').forEach(button => {
...
document.querySelectorAll('#mobile-menu button[onclick^="scrollToSection"]').forEach(button => {
    const sectionId = button.getAttribute('onclick').match(/'([^']+)'/)[1];
    button.onclick = () => scrollToSection(sectionId);
});
